from sqlalchemy import Integer, String, Text, ForeignKey, LargeBinary, JSON, Boolean
from sqlalchemy.orm import relationship, Mapped, mapped_column
# from backend.database import Base, str_uniq, int_pk, str_null_true, engine
from database import Base, str_uniq, int_pk, str_null_true, engine
...
class User(Base):
    ...
    phone: Mapped[str_uniq] =mapped_column(String, default="00000000" )
...
class Appointment(Base):
    ...
    docId: Mapped[int] = mapped_column(ForeignKey('doctor.id'), nullable=False)
    slotDate: Mapped[str] = mapped_column(String, nullable=False)
    slotTime: Mapped[str] = mapped_column(String, nullable=False)
    userData: Mapped[dict] = mapped_column(JSON, nullable=False)
    docData: Mapped[dict] = mapped_column(JSON, nullable=False)
