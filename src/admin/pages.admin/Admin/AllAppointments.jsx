...
const Appointments = () => {
    ...
    const fetchAppointments = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.log("active_appointments", data.active_appointments);
          console.log("history_appointments", data.history_appointments);
  ...
  const renderRow = (item, index, isHistory = false) => (
    ...
    >
      ...
      {/* Статус */}
      {item.cancelled ? (
        <p className="text-red-400 text-xs font-medium">{t.canceled}</p>
      ) : item.payment ? (
        <p className="text-green-500 text-xs font-medium">{t.onlineDone}</p>
      ) : item.isCompleted ? (
  ...
  return (
    <div className="w-full max-w-6xl m-5">
      <p className="mb-3 text-lg font-medium">{t.allapp}</p>
        ...
        {showCancelModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-lg w-full max-w-sm text-center shadow-lg">
              <p className="mb-4 text-lg font-medium">{t.areyousure}</p>
